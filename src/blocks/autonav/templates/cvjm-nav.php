...
foreach ($navItems as $ni) {
	...
	echo '<li class="nav-item ' . $ni->classes . '">'; //opens a nav item


	if ($ni->hasSubmenu && $ni->level == 1) {
		echo '<a href="' . $ni->url . '" target="' . $ni->target . '" class="nav-link dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false">' . $ni->name . '</a>';
	} else {
		if (!$ni->hasSubmenu) {
			echo '<a href="' . $ni->url . '" target="' . $ni->target . '" class="nav-link ' . $ni->classes . '">' . $ni->name . '</a>';
		} else {
			echo $ni->name;
		}
	}
